...
bool WithRowBrightened(const cv::Mat& gray, const std::vector<cv::Rect>& rows, int delta, cv::Mat& out)
{
    ...
    const cv::Rect patch = cv::Rect(x, row.y, row.width - (x - row.x), row.height) &
...
int main(int argc, char** argv)
{
    ...
    for (const fs::path& image : images)
    {
        ...
        if (cache.Misses() != 0)
        {
            failures.push_back({
                name,
                "the same frame re-read " + std::to_string(cache.Misses()) + " of " +
        ...
        if (WithOneRowEdited(gray, rows, edited))
        {
            ...
            if (cached != uncached)
            {
                failures.push_back({
                    ...
                    "      original rows: " + DescribeRows(rows) + "\n"
                    "      edited rows:   " + DescribeRows(ocr.FindLootRows(edited))
                });
            }
    ...
    if (driftPanels > 0)
    {
        std::printf("gradual drift caught after       %.1f steps of 2 levels, over %zu panels\n",
            static_cast<double>(driftSteps) / static_cast<double>(driftPanels), driftPanels);
    }
